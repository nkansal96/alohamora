...
def website_exists(experiment_name, results_dir):
    print("checking if already computed ", get_results_fname(experiment_name, results_dir) + ".json")
...
def test_website(args, checkpoint_file, experiment_name):
    f1 = get_results_fname(experiment_name, args.results_dir) + ".json"
    f2 = get_results_fname(experiment_name, args.results_dir) + ".log"
    f3 = get_results_fname(experiment_name, args.results_dir) + "-inference-time.out"
    print(f1)
    print(f2)
    with open(f1, "ab+") as outf:
        with open(f2, "ab+") as errf:
            monitor_process(
                [
                    "blaze",
                    "get_inference_time",
