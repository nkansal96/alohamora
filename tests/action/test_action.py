...
class TestActionSpace:
    ...
    def test_init_actions(self):
        ...
        assert len(
            set(
                (action.source.group_id, action.source.source_id, action.push.source_id)
        ...
        assert all(
            action.push.source_id > action.source.source_id
            for action in self.action_space.actions
            if not action.is_noop
        )
    ...
    def test_sample_returns_more_earlier_resources(self):
        ...
        actions = (
            (self.action_space.decode_action_id(action_id), size) for (action_id, size) in self.sampled_actions.items()
        )
        # remove no-ops
