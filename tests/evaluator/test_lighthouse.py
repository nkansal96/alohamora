...
def basic_pw_output_json():
    return json.dumps({"runs": [{"timings": [{"id": "speedIndex", "timing": 1000}]}]})


class TestParsePWOutput:
    def test_parse_pw_output_one_run(self):
        result = parse_pw_output({"runs": [{"timings": [{"id": "speedIndex", "timing": 1000}]}]})
    ...
    def test_parse_pw_output_multiple_runs(self):
        result = parse_pw_output(
            {
                "runs": [
                    {"timings": [{"id": "speedIndex", "timing": 1000}]},
                    {"timings": [{"id": "speedIndex", "timing": 1200}]},
                ]
            }
        )
        assert result.speed_index == 1000
    ...
    def test_parse_pw_output_with_median(self):
        result = parse_pw_output(
            {
                "median": {"timings": [{"id": "speedIndex", "timing": 1100}]},
                "runs": [
                    {"timings": [{"id": "speedIndex", "timing": 1000}]},
                    {"timings": [{"id": "speedIndex", "timing": 1200}]},
                ],
...
class TestGetMetrics:
    ...
    def test_get_metrics_reads_and_writes_files(self, mock_run, mock_open, mock_TemporaryDirectory):
        ...
        mock_run.return_value = subprocess.CompletedProcess(
            args=[], returncode=0, stdout=io.StringIO(basic_pw_output_json())
        )
    ...
    def test_get_metrics_spawns_subprocess_correctly(self, mock_run, mock_open, mock_TemporaryDirectory):
        ...
        mahimahi_cmd = self.mahimahi_config.proxy_replay_shell_with_cmd(
            "mm_push_policy", "trace", [self.config.pwmetrics_bin, "--config", "pw_config.js"]
        )
    ...
    def test_get_metrics_should_raise_if_subprocess_error(self, mock_run, mock_open):
        mock_run.return_value = subprocess.CompletedProcess(
            args=[], returncode=1, stdout=io.StringIO(basic_pw_output_json())
        )
