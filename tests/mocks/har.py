...
def get_har_json():
    x = json.loads(HAR_JSON)
    for entry in x["log"]["entries"]:
...
def generate_har():
    ...
    for (i, entry) in enumerate(har_entries):
        ...
        har_entries[i] = HarEntry(started_date_time=last_date, request=entry.request, response=entry.response, critical=False)
